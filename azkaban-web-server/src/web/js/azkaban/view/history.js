...
function retry(project, flow, execId){
  var isRetry=confirm("Are you sure to Retry?")
  if (isRetry == true){
      var url = contextURL + "/executor?ajax=executeFlow&project="+project+"&flow="+flow+"&oid="+execId+"&detype=historyRetry";
      $.ajax({
        type: "POST",
        url: url,
