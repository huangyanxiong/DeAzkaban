...
azkaban.JobEditView = Backbone.View.extend({
  events : {
    ...
    "click #exec-set-btn": "handleExecSet",
  ...
  showExec: function(projectName, flowName, jobName, execId) {
     this.projectName = projectName;
     this.flowName = flowName;
     this.jobName = jobName;
     this.execId = execId;
     ...
     var fetchJobExecInfo = {
       "project": this.projectName,
       "ajax": "fetchJobExecInfo",
       "flowName": this.flowName,
       "jobName": this.jobName,
       "execId": this.execId
     };
     var mythis = this;
     var fetchJobExecSuccessHandler = function(data) {
       if (data.error) {
         alert(data.error);
         return;
       }
     ...
     $.get(projectURL, fetchJobExecInfo, fetchJobExecSuccessHandler, "json");
  },
  ...
  handleExecSet: function(evt) {
     ...
     var jobOverrideData = {
       ...
       jobName: jobName,
       execId:execId,
       ajax: "setExecJobOverrideProperty",
       jobOverride: jobOverride
     };

     var projectURL = this.projectURL
     var redirectURL = projectURL+'?project='+project+'&flow='+flowName+'&job='+jobName+'&execId='+execId;
     var jobOverrideSuccessHandler = function(data) {
       if (data.error) {
         alert(data.error);
       }
