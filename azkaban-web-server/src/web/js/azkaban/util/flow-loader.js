...
var executeJob = function(jobId, flowId, evt) {
  //alert("execId:" + execId + ", jobId:" + jobId + ", flowId:" + flowId + ", projectName:" + projectName);
  var isRetry=confirm("Are you sure to Execute?")
  if (isRetry == true){
     var url = contextURL + "/executor?ajax=executeFlow&project="+projectName+"&flow="+flowId+"&oid="+execId+"&ojobId="+jobId+"&detype=nodeRetry";
     $.ajax({
        type: "POST",
        url: url,
        success: function(data) {
          location.href = contextURL + "/executor?execid="+data.execid;
        }
    });
  }
}
...
var nodeClickCallback = function(event, model, node) {
  ...
  if (type == "flow") {
    ...
    menu = [
      //{title: "Execute Job...", callback: function() {executeJob(jobId, flowId, event)}},
      //{break: 1},
...
var nodeExecClickCallback = function(event, model, node) {
  console.log("Node clicked callback");
  ...
  var requestURL = contextURL + "/manager?project=" + projectName + "&flow=" + flowId + "&job=" + jobId + "&execId=" + execId;
  var menu = [];
  ...
  if (type == "flow") {
    ...
    menu = [
      {title: "Execute Job...", callback: function() {executeJob(jobId, flowId, event)}},
      {break: 1},
      {title: "Open Job to Edit...", callback: function() {window.location.href=requestURL;}},
      {title: "Open Job in New Window...", callback: function() {window.open(requestURL);}},
      {break: 1},
      {title: "Center Job", callback: function() {model.trigger("centerNode", node)}}
    ];
