...
public class ProjectManagerServlet extends LoginAbstractAzkabanServlet {
  ...
  protected void handleGet(HttpServletRequest req, HttpServletResponse resp,
    ...
    if (hasParam(req, "project")) {
      if (hasParam(req, "ajax")) {
        ...
        handleJobExecPage(req, resp, session);
  ...
  private void handleAJAXAction(HttpServletRequest req,
    ...
    if (project == null) {
      ...
      } else if (ajaxName.equals("fetchJobExecInfo")) {
        if (handleAjaxPermission(project, user, Type.READ, ret)) {
          ajaxFetchJobExecInfo(project, ret, req);
        }
      } else if (ajaxName.equals("setJobOverrideProperty")) {
        if (handleAjaxPermission(project, user, Type.WRITE, ret)) {
          ajaxSetJobOverrideProperty(project, ret, req, user);
        }
      } else if (ajaxName.equals("setExecJobOverrideProperty")) {
        if (handleAjaxPermission(project, user, Type.WRITE, ret)) {
          ajaxExecSetJobOverrideProperty(project, ret, req);
        }
      } else {
  ...
  private void ajaxFetchJobExecInfo(Project project, HashMap<String, Object> ret,
       HttpServletRequest req) throws ServletException {
    String flowName = getParam(req, "flowName");
    String jobName = getParam(req, "jobName");
    ...
    Props prop;
    try {
      prop = executorManager.fetchExecutionJobInputProps(execId, jobName);
    } catch (ExecutorManagerException e) {
      ret.put("error", "Failed to retrieve job properties!");
      return;
    }
    ...
    ret.put("generalParams", generalParams);
    ret.put("overrideParams", generalParams);
  }
  ...
  private void ajaxExecSetJobOverrideProperty(Project project,
      HashMap<String, Object> ret, HttpServletRequest req)
      throws ServletException {
    String flowName = getParam(req, "flowName");
    String jobName = getParam(req, "jobName");
    ...
    Props overrideParams = new Props(null, jobParamGroup);
    try {
      executorManager.updateExecutionJobInputProps(execId, flow.getProjectId(), flow.getId(), jobName, overrideParams);
    } catch (ExecutorManagerException e) {
      ret.put("error", "Failed to upload job override property");
    }
  }
  ...
  private void handleJobExecPage(HttpServletRequest req, HttpServletResponse resp,
                                 Session session) throws ServletException {
    Page page =
            newPage(req, resp, session,
                    "azkaban/webapp/servlet/velocity/jobexecpage.vm");
    String projectName = getParam(req, "project");
    String flowName = getParam(req, "flow");
    ...
    try {
      ...
      Props props = executorManager.fetchExecutionJobInputProps(execId, jobName);
