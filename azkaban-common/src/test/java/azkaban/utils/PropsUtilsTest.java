...
public class PropsUtilsTest {
  ...
  @Test
  public void testLookupInterpolate() {
    Props props = Props.of("YYYY", "${DD:(YYYY)}", "YYYY-1", "${DD:(YYYY-1)}",
            "MM", "${DD:(MM)}", "MM-1", "${DD:(MM-1)}", "DD", "${DD:(DD)}", "DD-1", "${DD:(DD-1)}",
            "HH", "${DD:(HH)}", "HH-1", "${DD:(HH-1)}", "HH+2", "${DD:(HH+2)}", "mm", "${DD:(mm)}");
    Props resolved = PropsUtils.resolveProps(props);

    DateTime now = new DateTime();
    Assert.assertEquals(now.yearOfEra().getAsString(), resolved.get("YYYY"));
    Assert.assertEquals(now.plusYears(-1).yearOfEra().getAsString(), resolved.get("YYYY-1"));
    Assert.assertEquals(DateFormatLookup.paddingZero(now.monthOfYear().getAsString(), 2), resolved.get("MM"));
    Assert.assertEquals(DateFormatLookup.paddingZero(now.plusMonths(-1).monthOfYear().getAsString(), 2), resolved.get("MM-1"));
    Assert.assertEquals(DateFormatLookup.paddingZero(now.dayOfMonth().getAsString(), 2), resolved.get("DD"));
    Assert.assertEquals(DateFormatLookup.paddingZero(now.plusDays(-1).dayOfMonth().getAsString(), 2), resolved.get("DD-1"));
    Assert.assertEquals(DateFormatLookup.paddingZero(now.hourOfDay().getAsString(), 2), resolved.get("HH"));
    Assert.assertEquals(DateFormatLookup.paddingZero(now.plusHours(-1).hourOfDay().getAsString(), 2), resolved.get("HH-1"));
    Assert.assertEquals(DateFormatLookup.paddingZero(now.plusHours(2).hourOfDay().getAsString(), 2), resolved.get("HH+2"));
    Assert.assertEquals(DateFormatLookup.paddingZero(now.minuteOfHour().getAsString(), 2), resolved.get("mm"));
  }
