...
public class JdbcExecutorLoader extends AbstractJdbcLoader implements
  ...
  public void uploadExecutableNode(ExecutableNode node, Props inputProps)
    ...
    if (inputProps != null) {
      try {
        ...
        logger.info("inputProps ==> " + inputProps);
        inputParam = JSONUtils.toJSON(PropsUtils.toHierarchicalMap(PropsUtils.resolveProps(inputProps)));
  ...
  public void updateExecutableNode(ExecutableNode node)
    ...
    if (outputProps != null) {
      try {
        ...
        outputParam = JSONUtils.toJSON(PropsUtils.toHierarchicalMap(outputProps));
      } catch (Exception e) {
        throw new ExecutorManagerException("Error encoding input params");
      }
    }
  ...
  @Override
  public void updateExecutableNodeInputParam(int execId, int projectId, String flowId, String jobId, Props inputProps)
          throws ExecutorManagerException {
    final String UPSERT_EXECUTION_NODE_INPUTPARAM =
            "UPDATE execution_jobs "
                    + " SET input_params=? "
                    + "WHERE exec_id=? AND project_id=? AND flow_id=? AND job_id=?";
    String jsonString = null;
    if (inputProps != null) {
      try {
        jsonString = JSONUtils.toJSON(PropsUtils.toHierarchicalMap(inputProps));
      } catch (Exception e) {
        throw new ExecutorManagerException("Error encoding input params");
      }
    }
    ...
    QueryRunner runner = createQueryRunner();
    try {
      runner.update(UPSERT_EXECUTION_NODE_INPUTPARAM, jsonString, execId, projectId, flowId, jobId);
    } catch (SQLException e) {
      throw new ExecutorManagerException("Error updating job " + flowId, e);
    }
  }
