...
public class FlowRunner extends EventHandler implements Runnable {
  ...
  private void prepareJobProperties(ExecutableNode node) throws IOException {
    ...
    //Re execute job if originExecId is no equal 0
    String jobId = node.getId();
    int oid = this.flow.getOid();
    try {
      if (oid > 0) {
        Pair<Props, Props> pair = executorLoader.fetchExecutionJobProps(oid, jobId);

        if (pair != null){
          Props input = pair.getFirst();
          if (input != null){
            Map<String, Object> mapTemplate = PropsUtils.toHierarchicalMap(node.getInputProps());
            Map<String, Object> mapFinish = PropsUtils.toHierarchicalMap(input);

            Map<String, Object> mapTemplateProps = (Map)mapTemplate.get("props");
            Map<String, Object> mapFinishProps = (Map)mapFinish.get("props");
            for (String key : mapTemplateProps.keySet()) {
              Object value = mapFinishProps.get(key);
              ...
              mapTemplateProps.put(key, mapFinishProps.get(key));
            }
            mapTemplate.put("props", mapTemplateProps);
            node.setInputProps(PropsUtils.fromHierarchicalMap(mapTemplate));
          }

        } else {
          logger.error("pair is null, oid is " + oid + " and jobId is " + jobId);
        }
      }
    }catch(Exception ex){
      logger.error("cannot get jobProps.", ex);
    }
  }
